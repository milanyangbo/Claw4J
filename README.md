...
## Local Nacos For Service Discovery Tests

The runnable services default to `CLAW4J_NACOS_SERVER_ADDR=127.0.0.1:8848`, namespace `public`, group `CLAW4J_DEV_GROUP`, and environment `local`. For local smoke tests, use Docker to start a standalone Nacos 3.x server with embedded Derby storage. Pin the image to the current GA line `nacos/nacos-server:v3.2.4`; do not use the older `v2.5.1` image or floating `latest` tag for repeatable local tests.
